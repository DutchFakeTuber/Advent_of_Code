from Regolith_Reservoir import INPUT

def getData(input: str=INPUT) -> list[list]:
...
def partOne() -> int:
    _map: Map = Map(coords=getData(INPUT), start=[500, 0])
    _map.generate()
    while not _map.sand(): pass
    return _map.sandCounter

def partTwo() -> int:
    _map: Map = Map(coords=getData(INPUT), start=[500, 0], wideMap=True)
    _map.generate()
    while not _map.sand(): pass
    return _map.sandCounter
